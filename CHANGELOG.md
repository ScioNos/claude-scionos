...
- **Versioning**: Bumped the published package metadata to `4.3.4`.

### Fixed
- **Windows Doctor**: Avoid secure-store token decryption during `doctor` on Windows so diagnostics no longer crash after reporting stored LLM token status.
- **LLM Menu Availability**: Keep the single-model `claude-gpt-special` launch path selectable when RouterLab LLM reports `claude-gpt-5.4-sp`.
...
### Fixed
- **AWS Strategy Label**: Removed the hard-coded interactive `aws` menu alias so the selector now consistently shows `Claude via AWS (Opus 4.6, -50%)`.
...
### Fixed
- **Windows Secure Token Save**: Restrict the child `powershell.exe` `PSModulePath` to the native Windows PowerShell module directories so `ConvertTo-SecureString` and `ConvertFrom-SecureString` no longer autoload the incompatible PowerShell 7 security module during DPAPI token storage.
...
### Fixed
- **LLM Strategy Availability**: Treat non-exploitable RouterLab LLM model lists as unverified availability instead of blocking the entire launch with "No launchable strategy is available on RouterLab LLM."

## [4.1.7] - 2026-04-12

### Fixed
- **Windows Secure Token Save**: Encode the token in base64 inside the Windows PowerShell command so secure storage does not depend on stdin or environment-variable transport.

## [4.1.6] - 2026-04-12

### Fixed
- **Windows Secure Token Save**: Write the token to Windows PowerShell through stdin instead of an environment variable so secure storage works reliably with validated tokens.

## [4.1.5] - 2026-04-12

### Fixed
- **Windows Secure Token Handling**: Treat empty secure token files as missing/corrupted and fail fast when secure token writes produce no encrypted content.
...
[4.1.3]: https://github.com/ScioNos/claude-scionos/compare/v4.1.2...v4.1.3
